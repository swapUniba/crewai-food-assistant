...
def echo(user_message, history, actual_state, last_answer, food_info, user_preferences):
    ...
    if actual_state[-1] == 'awaiting_user_preferences':
        ...
        # Update state
        last_answer.append("Grazie per aver condiviso le tue preferenze! Adesso chiedimi qualcosa :)")
...
def first_interaction_handler(msg, actual_state, last_answer, food_info):
    ...
    if "1" in result:
        ...
        if "0" in food_info:
            # Try extracting food from the last bot message
            if last_answer:
                linkToPrevMsg = conversation_context_linker_crew.kickoff(inputs={'previous_message': last_answer[-1], 'current_message': msg})
                if "0" not in linkToPrevMsg:
                    food_info = last_answer[-1]
                    return second_interaction_handler(msg, actual_state, last_answer, food_info, '', True)
            # If still no food_info, proceed as before
...
def second_interaction_handler(msg, actual_state, last_answer, food_info, user_preferences, skipControl=False):
    if skipControl is False:
        print("ENTRA QUA")
